...
export async function getUserLoginInfo(pi, rol) {
  try {
    const query = 'select users.password , users.name, users.lastname, users.birthdate from users join userroles on users.pi = userroles.pi join roles on roles.id_role = userroles.id_role where users.pi = $1 and roles.name = $2;';
...
export async function getCollaboratorInfo(pi, rol) {
  try {
    const query = 'select users.password , users.name, users.lastname from users where users.pi = $1 and type_user= $2;';
    const result = await client.query(query, [pi, rol]);
    return result.rows[0]; // Devuelve solo el primer resultado
