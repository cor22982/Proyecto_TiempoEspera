...
export async function get_appointments(pi) {
  const result = await conn.query(`
    SELECT 
      a.id,                  -- Agregar el ID del appointment
      a.date::DATE, 
      a.time::TIME, 
      i.imagen, 
      ...
      p.name 
    FROM appointments a 
    JOIN userappointments us ON us."id appointment" = a.id 
    JOIN institutionsprocedures ip ON a."id institution procedure" = ip."id institution procedure" 
    JOIN intitutions i ON i.id_institutions = ip."id intitution" 
    JOIN procedures p ON p.id = ip."id procedure" 
    WHERE us.pi = $1 
